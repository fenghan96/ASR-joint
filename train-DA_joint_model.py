...
ATTENTION_OPTION = sys.argv[5] # "tanh" or "conv"
script_file = sys.argv[6]
save_dir = sys.argv[7] # save dir name
...
def onehot_da(labels, num_output):
    result = np.zeros(num_output, dtype='float32')
    result[int(labels)] = 1.0
...
def sort_pad(xs, ts, da, da_onehot, da_onehot_LS, ts_onehot, ts_onehot_LS,
             lengths, ts_lengths, da_lengths):
    """
    ...
    ts_result = []
    da_onehot_tensor = torch.zeros((BATCH_SIZE,da_maxlen,NUM_CLASSES_ACT),
                                   dtype=torch.float32,requires_grad=True).to(DEVICE)
    da_onehot_LS_tensor = torch.zeros((BATCH_SIZE,da_maxlen,NUM_CLASSES_ACT),
                                      dtype=torch.float32,requires_grad=True).to(DEVICE)
    da_result = []
    ...
    for i, i_sort in enumerate(arg_lengths):
        ...
        da_onehot_tensor.data[i, 0:da_lengths[i_sort]] = torch.from_numpy(da_onehot[i_sort])
        da_onehot_LS_tensor.data[i, 0:da_lengths[i_sort]] = torch.from_numpy(da_onehot_LS[i_sort])
        da_result.append(Variable(torch.from_numpy(da[i_sort]).to(DEVICE).long()))

    return xs_tensor, ts_result, da_result, da_onehot_tensor, da_onehot_LS_tensor, ts_onehot_tensor, ts_onehot_LS_tensor, lengths_tensor

def sort_pad_da(xs, lengths):
    arg_lengths = argsort(lengths)
    maxlen = max(lengths)
    
    xs_tensor = torch.zeros((BATCH_SIZE,int(maxlen), NUM_HIDDEN_NODES//2),
                            dtype=torch.float32,
                            requires_grad=True).to(DEVICE)
    lengths_tensor = torch.zeros((BATCH_SIZE), dtype=torch.float32, requires_grad=True).to(DEVICE)

    for i, i_sort in enumerate(arg_lengths):
        xs_tensor.data[i] = torch.from_numpy(xs[i_sort])
        lengths_tensor.data[i] = lengths[i_sort]
...
class Model(nn.Module):
    def __init__(self):
        ...
        # word-level encoder
        self.dalstm = nn.LSTM(input_size=NUM_HIDDEN_NODES,
                              hidden_size=NUM_HIDDEN_NODES//4, num_layers=2,
                              batch_first=True,dropout=0.2, bidirectional=True)
        self.seglstm = nn.LSTM(input_size=NUM_HIDDEN_NODES//2,
                               hidden_size=NUM_HIDDEN_NODES//2, num_layers=1,
                               batch_first=True)
        self.nda = nn.Linear(NUM_HIDDEN_NODES//2, NUM_CLASSES_ACT)

    def forward(self, x, lengths, target, da):
        ...
        n = torch.zeros((batch_size, num_labels, NUM_HIDDEN_NODES), device=DEVICE,
                        requires_grad=False)

        youtput = torch.zeros((batch_size, num_labels, NUM_CLASSES), device=DEVICE, requires_grad=False)
        Youtput = torch.zeros((batch_size, num_labels, NUM_CLASSES_ACT),
        ...
        word_level, _ = self.dalstm(n)
        word_lengths = np.zeros(batch_size)
        maxlen_da = 0
        for step1 in range(batch_size):
            num_index = 0
            for step2 in range(num_labels):
                if arg_onehot(target.data[step1][step2])==27287:
                    num_index += 1
            if maxlen_da < num_index:
                maxlen_da = num_index
        
        #print(maxlen_da)
        word_level_output = np.zeros((batch_size, maxlen_da, NUM_HIDDEN_NODES//2))
        for step1 in range(batch_size):
            num_index = 0
            for step2 in range(num_labels):
                if arg_onehot(target.data[step1][step2])==27287:
                    word_level_output[step1,num_index] = word_level.data[step1,step2] 
                    num_index += 1
            word_lengths[step1] = num_index 
        
        word_level_output, word_lengths = sort_pad_da(word_level_output,word_lengths)
        total_length_da = word_level_output.size(1)
        seg_x = nn.utils.rnn.pack_padded_sequence(word_level_output, word_lengths, batch_first=True)
        
        #self.bi_lstm.flatten_parameters()
        seg_level_output, _ = self.seglstm(segx)
        seg_batch, word_lengths = nn.utils.rnn.pad_packed_sequence(seg_level_output,batch_first=True, total_length=total_length_da)

        Youtput = self.nda(seg_batch)
...
# load
#model.load_state_dict(load_model("model.word.mb40_LS_APSSPS_risk.tts_sps/network.epoch13"))
#optimizer.load_state_dict(torch.load("model.word.mb40_LS_APSSPS_risk.tts_sps/network.optimizer.epoch13"))
...
for epoch in range(0, 30):
    ...
    for i in range(num_mb):
        ...
        for j in range(BATCH_SIZE):
            ...
            _,x_file,_, _, _, _, daorg, _, laborg, _= s.split('\t')
            if '.htk' in x_file:
                cpudat = load_dat(x_file)
                cpudat = cpudat[:, :40]
                newlen = int(cpudat.shape[0])
            elif '.npy'in x_file:
                cpudat = np.load(x_file)
                newlen = int(cpudat.shape[0])

#            print("{} {}".format(x_file, cpudat.shape[0]))
            if FRAME_STACKING and '.htk' in x_file:
            ...
            dialog_act = np.array([int(i.strip(',')) for i in daorg.split(',')],dtype=np.int32)
            ts.append(cpulab)
            ...
            dialog_act_onehot = onehot(dialog_act, NUM_CLASSES_ACT)
            da.append(dialog_act)
            da_onehot_LS.append(0.9*dialog_act_onehot+0.1*1.0/NUM_CLASSES_ACT)
            da_onehot.append(dialog_act_onehot)
            da_lengths.append(len(dialog_act))

        xs, ts, da, da_onehot, da_onehot_LS, ts_onehot, ts_onehot_LS, lengths = sort_pad(xs, ts, da, da_onehot, da_onehot_LS, ts_onehot, ts_onehot_LS, lengths, ts_lengths,da_lengths)

        youtput_in_Variable, Youtput_in_Variable = model(xs, lengths,
                                                         ts_onehot, da_onehot)
        ...
        for i in range(BATCH_SIZE):
            ...
            num_da = da[i].size(0)
            loss += -(F.log_softmax(youtput_in_Variable[i][:num_labels], dim=1)
                      * ts_onehot_LS[i][:num_labels]).sum() / num_labels * 0.9
            loss += -(F.log_softmax(Youtput_in_Variable[i][:num_da],
                                    dim=0) * da_onehot_LS[i][:num_da]).sum() /num_labels * 0.1
