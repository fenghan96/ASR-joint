...
def sort_pad(xs, ts, da, da_onehot, da_onehot_LS, ts_onehot, ts_onehot_LS, lengths, ts_lengths):
    ...
    ts_result = []
    da_onehot_tensor = torch.zeros((BATCH_SIZE,NUM_CLASSES_ACT),
                                   dtype=torch.float32,requires_grad=True).to(DEVICE)
    da_onehot_LS_tensor = torch.zeros((BATCH_SIZE,NUM_CLASSES_ACT),
                                      dtype=torch.float32,requires_grad=True).to(DEVICE)
    da_result = []
    ...
    for i, i_sort in enumerate(arg_lengths):
        ...
        da_onehot_tensor.data[i] = torch.from_numpy(da_onehot[i_sort])
        da_onehot_LS_tensor.data[i] = torch.from_numpy(da_onehot_LS[i_sort])

    return xs_tensor, ts_result, da_onehot_tensor, da_onehot_LS_tensor, ts_onehot_tensor, ts_onehot_LS_tensor, lengths_tensor
...
class Model(nn.Module):
    def __init__(self):
        ...
        # word-level encoder
        self.sn = nn.Linear(NUM_HIDDEN_NODES, NUM_HIDDEN_NODES, bias=False)
        self.nn = nn.Linear(NUM_HIDDEN_NODES, NUM_HIDDEN_NODES)
        self.wn = nn.Linear(NUM_HIDDEN_NODES, NUM_HIDDEN_NODES)
        self.dan = nn.Linear(NUM_CLASSES_ACT, NUM_HIDDEN_NODES)
        self.nda = nn.Linear(NUM_HIDDEN_NODES, NUM_CLASSES_ACT)
    ...
    def forward(self, x, lengths, target, da):
        ...
        n = torch.zeros((batch_size, NUM_HIDDEN_NODES), device=DEVICE,
                        requires_grad=False)
...
for epoch in range(0, 30):
    ...
    for i in range(num_mb):
        ...
        for j in range(BATCH_SIZE):
            ...
            _,x_file,_, _, _, _, dialog_act, _, laborg, _= s.split('\t')
            if '.htk' in x_file:
...
#            print("{} {}".format(x_file, cpudat.shape[0]))
        ...
        xs, ts, da_onehot, da_onehot_LS, ts_onehot, ts_onehot_LS, lengths = sort_pad(xs, ts, da, da_onehot, da_onehot_LS, ts_onehot, ts_onehot_LS, lengths, ts_lengths)
        ...
        for i in range(BATCH_SIZE):
            ...
            loss += -(F.log_softmax(Youtput_in_Variable[i][num_labels-1],
                                    dim=0) * da_onehot_LS[i]).sum() /num_labels * 0.1
